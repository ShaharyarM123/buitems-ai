...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = (key, setter, isJson = true) => {
      const val = localStorage.getItem(key);
      if (val) setter(isJson ? JSON.parse(val) : val);
    };

    load('buitems_mock_tests', setSavedTests);
    load('buitems_test_histories', setTestHistories);
    load('buitems_personal_bookshelves', setBookshelves);
    load('buitems_active_bookshelf_id', setActiveBookshelfId, false);
    load('buitems_theme_mode', setThemeMode, false);
    load('buitems_user_name', (v) => { setUserName(v); setTempName(v); }, false);
  }, [router]);
  ...
  const handleSaveName = () => {
    ...
    if (!trimmed) return;
    setUserName(trimmed);
    updateStorage('buitems_user_name', trimmed);
  ...
  const saveItemToBookshelf = (shelfId, item) => {
    const updated = bookshelves.map(shelf => 
      shelf.id === shelfId ? { ...shelf, items: [item, ...shelf.items] } : shelf
    );
    setBookshelves(updated);
    updateStorage('buitems_personal_bookshelves', updated);
  ...
  const handleCreateNewBookshelfAndSave = () => {
    ...
    const newShelf = {
      ...
      date,
      items: pendingItemToSave ? [{ text: pendingItemToSave.text, title: pendingItemToSave.title, date }] : []
    };

    const updatedShelves = [newShelf, ...bookshelves];
    setBookshelves(updatedShelves);
    setActiveBookshelfId(shelfId);
    updateStorage('buitems_personal_bookshelves', updatedShelves);
    updateStorage('buitems_active_bookshelf_id', shelfId);
  ...
  const handleDeleteBookshelf = (e, shelfId) => {
    ...
    if (window.confirm("Are you sure you want to delete this bookshelf?")) {
      const updated = bookshelves.filter(s => s.id !== shelfId);
      setBookshelves(updated);
      updateStorage('buitems_personal_bookshelves', updated);
      ...
      if (viewingBookshelf?.id === shelfId) setViewingBookshelf(null);
    }
  };
  ...
  const handleDeleteTest = (e, testId) => {
    ...
    updateStorage('buitems_mock_tests', updated);
  ...
  const extractAndSavedMockTestSafe = (rawText, originalPrompt) => {
    ...
    try {
      ...
      console.warn("JSON parse error safely caught.");
    }

    if (questions.length === 0) {
      questions = Array.from({ length: 50 }, (_, i) => ({
        question: `Question ${i + 1}: What is a major aspect or concept related to ${topicName}?`,
        options: [`Fundamental principle ${i + 1}`, `Secondary framework`, `Historical context`, `None of the above`],
        correct: 0,
        topic: topicName
      }));
    }

    const newTest = { id: Date.now().toString(), title: topicName, date: new Date().toLocaleDateString(), questions };
    const updatedTests = [newTest, ...savedTests];
    setSavedTests(updatedTests);
    updateStorage('buitems_mock_tests', updatedTests);
  ...
  const handleSubmitTest = () => {
    ...
    const previous = testHistories[activeTest.id];
    
    let comparisonMessage = "First Attempt Completed!";
    let statusColor = "text-zinc-600";

    if (previous) {
      if (percentage > previous.lastPercentage) {
        comparisonMessage = `Improved! Score went up to ${percentage}% (from ${previous.lastPercentage}%).`;
        statusColor = "text-emerald-600 font-bold";
      } else if (percentage < previous.lastPercentage) {
        comparisonMessage = `Score dropped to ${percentage}% (from ${previous.lastPercentage}%).`;
    ...
    setTestResult({ score, total: activeTest.questions.length, percentage, weakTopics, comparisonMessage, statusColor });
    ...
    updateStorage('buitems_test_histories', updatedHistory);
  };

  const handleRetakeTest = () => {
    const shuffledQuestions = activeTest.questions.map(q => {
      const optionsWithIndex = q.options.map((text, isCorrect) => ({ text, isCorrect: isCorrect === q.correct }))
  ...
  const handleImageUpload = (fileList) => {
    ...
    files.forEach(file => {
      ...
      reader.onload = (e) => setNoteExpanderImages(prev => [...prev, {
  ...
  const removeImage = (id) => setNoteExpanderImages(prev => prev.filter(img => img.id !== id));
  ...
  const handlePdfUpload = (fileList) => {
    ...
    const reader = new FileReader();
    reader.onload = (e) => setSelectedPdf({ name: file.name, size: file.size, base64: e.target.result });
    reader.readAsDataURL(file);
  };
  ...
  const handleDrag = (setter) => (e) => { e.preventDefault(); setter(true); };
  const handleDragLeave = (setter) => (e) => { e.preventDefault(); setter(false); };
  const handleDrop = (setter, handler) => (e) => {
    e.preventDefault();
    setter(false);
  ...
  const handleSendQuery = async (e, customPrompt = null) => {
    ...
    const hasImages = activeTab === 'Note Expander' && noteExpanderImages.length > 0;
    const hasPdf = activeTab === 'PDF Uploader' && !!selectedPdf;

    if ((!queryText && !hasImages && !hasPdf) || loading) return;

    const userMessageText = queryText || (hasImages ? `📎 Uploaded ${noteExpanderImages.length} image(s) of notes` : `📄 Uploaded ${selectedPdf.name}`);
    const userMessage = { id: Date.now().toString(), sender: 'user', text: userMessageText };
    ...
    try {
      ...
      if (hasPdf) { payload.pdf = selectedPdf.base64; payload.pdfName = selectedPdf.name; }
      ...
      setToolStates(prev => ({
        ...prev,
        [activeTab]: { ...prev[activeTab], messages: [...prev[activeTab].messages, { id: Date.now().toString(), sender: 'ai', text: `Error: ${err.message}` }] }
      }));
    } finally {
      setLoading(false);
    }
                  ...
                  currentToolState.messages.map((msg) => (
                    msg.sender === 'user' ? (
                      <div key={msg.id} className="flex justify-end w-full">
                        <div className="max-w-[90%] sm:max-w-[80%] bg-zinc-100 text-zinc-900 border border-zinc-200 px-4 py-3 sm:px-5 sm:py-3.5 rounded-2xl rounded-br-none text-sm sm:text-base flex items-start gap-3 shadow-sm">
            ...
            <div className="p-3 sm:p-6 bg-transparent">
              <div className="max-w-4xl mx-auto space-y-3">
                ...
                {activeTab === 'Note Expander' && (
                  <div
                    onDragOver={handleDrag(setIsDraggingOver)}
                    onDragLeave={handleDragLeave(setIsDraggingOver)}
                    onDrop={handleDrop(setIsDraggingOver, handleImageUpload)}
                ...
                {activeTab === 'PDF Uploader' && (
                  <div
                    onDragOver={handleDrag(setIsPdfDraggingOver)}
                    onDragLeave={handleDragLeave(setIsPdfDraggingOver)}
                    onDrop={handleDrop(setIsPdfDraggingOver, handlePdfUpload)}
                    className={`rounded-2xl border border-dashed transition-all p-4 bg-zinc-50 ${
                      isPdfDraggingOver
                        ? 'border-emerald-500 bg-emerald-50/50'
                        : 'border-zinc-300 hover:border-zinc-400'
...
//   // --- RENDER (UI LAYOUT REMAINS EXACTLY IDENTICAL) ---
//   return (
//     <div className="flex h-[100dvh] bg-white text-black font-sans overflow-hidden relative selection:bg-emerald-500 selection:text-white">
